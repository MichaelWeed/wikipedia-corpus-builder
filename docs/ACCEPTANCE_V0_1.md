...
**Criterion 19 status**: genuinely `NOT_RUN`, not just undocumented — verifying it requires GitHub Actions to actually execute `desktop.yml`/`release.yml` on real macOS/Windows/Linux runners, which needs a push to a branch with an open PR against `main` (or a tag push for `release.yml`). Neither has happened yet in this session; per this project's git safety rules, pushing is a user-authorized action, not something to do unprompted. `desktop.yml` and `release.yml` were both updated 2026-08-14 to add the Linux Tauri system-dependency install step they were previously missing (webkit2gtk/appindicator/etc. — `ubuntu-latest` doesn't have these preinstalled, so the Linux leg of this matrix has likely never actually compiled), but that fix itself is unverified against real CI for the same reason.
